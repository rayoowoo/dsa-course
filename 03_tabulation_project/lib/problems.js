...
function stepper(nums) {
    for (let i = 1; i < nums.length; i++) {
        if (nums.slice(0, i).some(el => i <= el) === false) {
            return false;
        }
    }
    return true;
...
//         const toAdd = table[i -2] > table[i-3] ? table[i - 2] : table[i - 3];
//         table[i] = toAdd + nums[i];
//     }
//     const length = table.length - 1;
//     return table[length] > table[length - 1] ? table[length] : table[length - 1];
// }

function maxNonAdjacentSum(nums, top = true, memo = {}) {
    ...
    const tryOne = maxNonAdjacentSum(nums.slice(0, length - 2), false, memo)
    const tryTwo = maxNonAdjacentSum(nums.slice(0, length - 3), false, memo)
    const attempt = tryOne > tryTwo ? nums[length -1] + tryOne : nums[length-1] + tryTwo;
    if ( attempt > option) option = attempt;

    if (top) {
        const other = maxNonAdjacentSum(nums.slice(0, length - 1), false, memo);
        if (other > option) option = other;
    }
